...
public class BridgeSizeValidator {
    public static void validate(String bridgeSize) {
        if (!hasValidType(bridgeSize)) {
            throw new IllegalArgumentException(INVALID_BRIDGE_SIZE_TYPE_ERROR.getMessage());
        }

        if (!hasValidRange(Integer.parseInt(bridgeSize))) {
            throw new IllegalArgumentException(INVALID_BRIDGE_SIZE_RANGE_ERROR.getMessage());
        }
    }
